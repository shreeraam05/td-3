...
# Save to both primary and mirror databases synchronously
def save_db(db):
    with open("database.json", "w") as f:
        json.dump(db, f, indent=2)
    with open("mirror_database.json", "w") as f:
        json.dump(db, f, indent=2)

# Load with failover to mirror if primary fails
def load_db_with_failover():
    try:
        return load_db()
    except FileNotFoundError:
        with open("mirror_database.json", "r") as f:
            return json.load(f)

# Products Routes
@app.route('/products', methods=['GET'])
def get_products():
    db = load_db_with_failover()
