...
def save_db(db):
    ...
    replication_queue.append(db.copy())

def init_sqlite():
    conn = sqlite3.connect("replica.db")
    c = conn.cursor()
    c.execute("CREATE TABLE IF NOT EXISTS products (id INTEGER, name TEXT, price REAL)")
    conn.commit()
    conn.close()

def replicate_to_sqlite(db):
    conn = sqlite3.connect("replica.db")
    c = conn.cursor()
    c.execute("DELETE FROM products")
    for p in db['products']:
        c.execute("INSERT INTO products VALUES (?, ?, ?)", (p['id'], p['name'], p['price']))
    conn.commit()
    conn.close()
...
def replicate():
    while True:
        if replication_queue:
            db = replication_queue.pop(0)
            with open("replica_database.json", "w") as f:
                json.dump(db, f, indent=2)
            replicate_to_sqlite(db)
        time.sleep(5)
